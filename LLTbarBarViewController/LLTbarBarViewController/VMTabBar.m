...
-(void)clickTab:(UITapGestureRecognizer*)sender
{
    ...
    [self animationTabbarWithTab:tabView]; //动画效果
    [self.scrollView setContentOffset:CGPointMake(0, CGRectGetHeight(self.bounds)*(tabView.tag-1)) animated:NO];//切换显示 view
    [tabView setUserInteractionEnabled:NO];
    
    [self.delegate VMTabBar:self switchTabWithTag:tabView.tag];//通知已选择的tabBar
